  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-[#1B3C53]">{t('provider.booking_history', 'Booking History')}</h2>
        ...
        {bookings.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto p-3 bg-white rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 text-[#1B3C53] font-bold px-2">
              <Filter size={18} />
              <span className="text-sm">{t('bookings.filter_by', 'Filter')}</span>
            </div>
            <select
              value={statusFilter}
            ...
            >
              <option value="all">{t('bookings.all_statuses', 'All Statuses')}</option>
            ...
            >
              {uniqueServices.map(service => (
                <option key={service} value={service}>
                  {service === 'all' ? t('bookings.all_services', 'All Services') : t(`service_names.${service}`)}
                </option>
              ))}
            </select>
            <select
              ...
              className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1B3C53] bg-gray-50 text-gray-700"
            >
              <option value="all">{t('bookings.any_time', 'Any Time')}</option>
              <option value="today">{t('bookings.today', 'Today')}</option>
              <option value="this_week">{t('bookings.this_week', 'This Week')}</option>
              <option value="this_month">{t('bookings.this_month', 'This Month')}</option>
            </select>
          </div>
        )}
      </div>
            ...
            <div className="flex justify-between items-start gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="font-bold text-[#1B3C53] text-lg">{t(`service_names.${booking.service_name_key}`)}</h3>
                  ...
                  {booking.is_rework && (
                    <span className="px-2 py-1 bg-red-600 text-white text-[10px] font-bold rounded uppercase">
                      {t('bookings.rework_insurance', 'Rework')}
                    </span>
                  )}
                  {booking.status === 'paid' && (
                    <span className="ml-2 px-2 py-1 rounded-full text-xs font-semibold uppercase bg-blue-600 text-white">
                      {booking.payment_method === 'cash' ? t('provider.confirm_cash', 'Cash') : t('bookings.paid_online', 'Online')}
                    </span>
                  )}
                </div>
